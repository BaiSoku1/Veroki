...
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/BaiSoku1/Fluent-Modded/refs/heads/master/Fluent/Main.lua"))()
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/BaiSoku1/Fluent-Modded/master/Addons/SaveManager.lua"))()
local InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/BaiSoku1/Fluent-Modded/master/Addons/InterfaceManager.lua"))()
...
if type(executor) == "table" then for key, value in pairs(executor) do print(key .. ": " .. tostring(value)) end elseif type(executor) == "string" then print("Executor: " .. executor) end
...
MainTab:AddToggle("LupenESPToggle", {Title = "ESP Lupen",Default = false,Callback = function(state) if state then LupenESPModule:Start() else LupenESPModule:Stop() end end})
    ...
    while true do
        ...
        if getgenv().featureStates.AntiNextbotSpawn then
            ...
            if currentAttachment and not cachedAttachment then
                ...
                if Fluent and Fluent.Notify then Fluent:Notify({Title = "Anti Nextbot Spawn",Content = "Attachment found! System now active.",Duration = 3}) end
            elseif not currentAttachment and cachedAttachment then
                cachedAttachment = nil
                if Fluent and Fluent.Notify then Fluent:Notify({Title = "Anti Nextbot Spawn",Content = "Attachment lost - System will reactivate when found.",Duration = 3}) end
            else
                cachedAttachment = currentAttachment
            end
        end
...
if Fluent and Fluent.Notify then Fluent:Notify({Title = "Anti Nextbot System",Content = "System loaded successfully!",Duration = 2}) end
restartSystemOnEvents()

CombatTab:AddToggle("AntiNextbot", {Title = "Anti-Nextbot",Description = "Automatically teleport away from nearby Nextbots (farms pause if too close)",Default = getgenv().featureStates.AntiNextbot,Callback = function(state) getgenv().featureStates.AntiNextbot = state if state then startAntiNextbot() else stopAntiNextbot() end end})
...
VisualTab:AddInput("AvatarInput", {Title = "Username / User ID",Description = "Enter Roblox username or user ID",Default = "",Placeholder = "Roblox or 123456789",Callback = function(value) avatarInputValue = value if value and value ~= "" then changeAvatar(value) end end})
VisualTab:AddButton({Title = "Apply Avatar",Description = "Apply avatar from entered username/ID",Callback = function() if avatarInputValue and avatarInputValue ~= "" then changeAvatar(avatarInputValue) end end})
...
VisualTab:AddToggle("CameraStretch", {Title = "Camera Stretch",Description = "Enable camera stretch effect",Default = false,Callback = function(state)
    if state then updateCameraStretch() else if cameraStretchConnection then cameraStretchConnection:Disconnect() cameraStretchConnection = nil end end
end})
...
VisualTab:AddInput("FakeStreaks", {Title = "Fake Streaks",Description = "Set fake streak value",Default = "",Placeholder = "Enter streak value",Numeric = true,Callback = function(value)
    ...
    if num then
        pcall(function()
            ...
            if Fluent and Fluent.Notify then Fluent:Notify({Title = "Streak",Content = string.format("Streak set to %d", num),Duration = 3}) end
        end)
    end
end})
...
VisualTab:AddInput("CurrentCosmetics", {Title = "Current Cosmetics",Description = "Enter current cosmetic name",Default = "",Placeholder = "Enter current cosmetic name",Callback = function(v) cosmetic1 = v if not isSwapped then originalCosmetic1 = v end end})

VisualTab:AddInput("SelectCosmetics", {Title = "Select Cosmetics",Description = "Enter cosmetic to swap with",Default = "ToxicInferno",Placeholder = "Enter cosmetic to swap with",Callback = function(v) cosmetic2 = v if not isSwapped then originalCosmetic2 = v end end})
...
AutoFarmsTab:AddToggle("AntiAFK", {Title = "Anti AFK",Description = "Prevent being kicked for AFK",Default = false,Callback = function(state) getgenv().AntiAFKEnabled = state if state then startAntiAFK() end end})
...
AutoFarmsTab:AddToggle("AutoLupenToggle", {Title = "Auto Farm Lupen",Default = false,Callback = function(state) AutoLupenEnabled = state if state then startAutoLupen() else stopAutoLupen() end end})
