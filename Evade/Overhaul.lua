...
mt[idx]=function(...)
if checkcaller() thenreturn func(...) elsereturn old and old(...) end
end
...
if not getgenv().Fluent thengetgenv().Fluent=Fluent end
...
a, b=a:lower(), b:lower()
local m, n=#a, #b
if m==0 thenreturn n end
if n==0 thenreturn m end
local matrix={}
...
if type(executor)=="table" thenfor key, value in pairs(executor) do print(key .. ": " .. tostring(value)) endelseiftype(executor)=="string" thenprint("Executor: " .. executor) do end
local MainTab=Window:AddTab({Title="Main",Icon="home"})
local AutoFarmsTab=Window:AddTab({Title="Auto Farms",Icon="coins"})
local CombatTab=Window:AddTab({Title="Combat",Icon="sword"})
local MiscTab=Window:AddTab({Title="Misc",Icon="star"})
local VisualTab=Window:AddTab({Title="Visual",Icon="diamond"})
local InfoTab=Window:AddTab({Title="Info",Icon="info"})
local ConfigTab=Window:AddTab({Title="Configuration",Icon="save"})
local ExtensionTab=Window:AddTab({Title="Extension",Icon="puzzle"})
if not getgenv().featureStates thengetgenv().featureStates={} end
...
if not getgenv().espObjects thengetgenv().espObjects={} end
if not getgenv().espThreads thengetgenv().espThreads={} end
if not getgenv().tracerLines thengetgenv().tracerLines={} end
...
InfoTab:AddParagraph({Title="Veroki Hub Discord! :]",Content="Join our Discord server for updates, support, and community!"})
InfoTab:AddButton({Title="Copy Discord Link",Description="Click to copy Discord invite link",Callback=function() setclipboard(getgenv().DiscordInfo.ServerLink) Fluent:Notify({Title="Discord",Content="Discord link copied to clipboard!",Duration=3}) getgenv().DiscordInfo.NotificationShown=true end})
InfoTab:AddButton({Title="Join Server",Description="Teleport to Discord server",Callback=function() pcall(function() game:GetService("TeleportService"):Teleport(getgenv().DiscordInfo.ServerID, LocalPlayer) end) end})
InfoTab:AddParagraph({Title="Credits",Content="Script created by SYFTA\nUI Library: Fluent Modded\nSpecial thanks to all testers and supporters!"})
local function createESP(part, name, baseColor)
if getgenv().espObjects[part] thenreturn getgenv().espObjects[part] end
...
if data and data.billboard thendata.billboard:Destroy() end
end
getgenv().espObjects={}
for name, thread in pairs(getgenv().espThreads) do
if thread thencoroutine.close(thread) getgenv().espThreads[name]=nil end
end
end
MainTab:AddToggle("ESPPlayer", {Title="ESP Player",Description="Show player ESP",Default=false,Callback=function(state)
getgenv().featureStates.EspPlayer=state
if getgenv().espThreads["player"] thencoroutine.close(getgenv().espThreads["player"]) getgenv().espThreads["player"]=nil end
for part, data in pairs(getgenv().espObjects) do if data and data.type=="Player" thenremoveESP(part) do endend
if not state thenreturn end
...
if not char:IsA("Model") thencontinue end
if char:GetAttribute("Team")=="Nextbot" thencontinue end
if char.Name==LocalPlayer.Name thencontinue end
local hrp=char:FindFirstChild("HumanoidRootPart")
if not hrp thencontinue end
local espData=getgenv().espObjects[hrp]
if not espData thenespData=createESP(hrp, "Player", Color3.fromRGB(100, 180, 255)) end
if not espData or not espData.billboard or not espData.billboard.Parent thengetgenv().espObjects[hrp]=nil continue end
...
MainTab:AddToggle("ESPTicket", {Title="ESP Ticket",Description="Show ticket ESP",Default=false,Callback=function(state)
getgenv().featureStates.EspTicket=state
if getgenv().espThreads["ticket"] thencoroutine.close(getgenv().espThreads["ticket"]) getgenv().espThreads["ticket"]=nil end
for part, data in pairs(getgenv().espObjects) do if data and data.type=="Ticket" thenremoveESP(part) do endend
if not state thenreturn end
...
if not ticketModel:IsA("Model") thencontinue end
local part=ticketModel:FindFirstChildWhichIsA("BasePart")
if not part thencontinue end
local espData=getgenv().espObjects[part]
if not espData thenespData=createESP(part, "Ticket", Color3.fromRGB(255, 255, 100)) end
if not espData or not espData.billboard or not espData.billboard.Parent thengetgenv().espObjects[part]=nil continue end
...
MainTab:AddToggle("ESPNextbot", {Title="ESP Nextbot",Description="Show nextbot ESP",Default=false,Callback=function(state)
getgenv().featureStates.EspNextbot=state
if getgenv().espThreads["nextbot"] thencoroutine.close(getgenv().espThreads["nextbot"]) getgenv().espThreads["nextbot"]=nil end
for part, data in pairs(getgenv().espObjects) do if data and data.type=="Nextbot" thenremoveESP(part) do endend
if not state thenreturn end
...
if not npc:IsA("Model") thencontinue end
if npc:GetAttribute("Team")~="Nextbot" thencontinue end
...
if not part thencontinue end
local espData=getgenv().espObjects[part]
if not espData thenespData=createESP(part, "Nextbot", Color3.fromRGB(200, 100, 255)) end
if not espData or not espData.billboard or not espData.billboard.Parent thengetgenv().espObjects[part]=nil continue end
local dist=getDistance(part.Position)
...
for _, line in pairs(getgenv().tracerLines) do if line thenpcall(function() line:Remove() end) do endend
getgenv().tracerLines={}
end
MainTab:AddToggle("TracerDowned", {Title="Tracer Downed Players",Description="Show tracers to downed players",Default=false,Callback=function(state)
getgenv().featureStates.TracerDowned=state
if getgenv().tracerThread thencoroutine.close(getgenv().tracerThread) getgenv().tracerThread=nil end
cleanupTracers()
if not state thenreturn end
...
if char:GetAttribute("Team")=="Nextbot" thencontinue end
if char.Name==LocalPlayer.Name thencontinue end
if char:GetAttribute("Downed")~=true thencontinue end
local hrp=char:FindFirstChild("HumanoidRootPart")
if not hrp thencontinue end
...
MainTab:AddToggle("LupenESPToggle", {Title="ESP Lupen",Default=false,Callback=function(state) if state thenLupenESPModule:Start() elseLupenESPModule:Stop() endend})
...
if AutoVoteConnection thenAutoVoteConnection:Disconnect() end
...
MainTab:AddDropdown("AutoVoteMap", {
Title="Auto Vote Map",
...
if value=="Map 1" then
getgenv().featureStates.SelectedMap=1
elseifvalue=="Map 2" then
getgenv().featureStates.SelectedMap=2
elseifvalue=="Map 3" then
getgenv().featureStates.SelectedMap=3
elseifvalue=="Map 4" then
getgenv().featureStates.SelectedMap=4
end
end
})
MainTab:AddToggle("AutoVoteToggle", {
Title="Auto Vote",
Default=false,
...
MainTab:AddDropdown("AutoVoteMode", {
Title="Vote Mode",
...
getgenv().featureStates.SelectedVoteMode=1
elseifvalue=="Mode 2" then
getgenv().featureStates.SelectedVoteMode=2
elseifvalue=="Mode 3" then
getgenv().featureStates.SelectedVoteMode=3
elseifvalue=="Mode 4" then
getgenv().featureStates.SelectedVoteMode=4
end
end
})
MainTab:AddToggle("AutoVoteModeToggle", {
Title="Auto Vote Game Mode",
Default=false,
Callback=function(state)
getgenv().featureStates.AutoVoteMode=state
if state then
if AutoVoteModeConnection thenAutoVoteModeConnection:Disconnect() end
...
if bv thenbv:Destroy() end
if bg thenbg:Destroy() end
end
if speaker.Character and speaker.Character:FindFirstChildWhichIsA("Humanoid") then
speaker.Character:FindFirstChildWhichIsA("Humanoid").PlatformStand=false
end
if mfly1 thenmfly1:Disconnect() mfly1=nil end
if mfly2 thenmfly2:Disconnect() mfly2=nil end
disableSpeedMonitor()
end)
end
...
if FLYING thenflyUpPressed=true end
end)
jumpButton.MouseButton1Up:Connect(function()
if FLYING thenflyUpPressed=false end
end)
end
end
end
...
if FLYING thenflyDownPressed=true end
end)
crouchButton.MouseButton1Up:Connect(function()
if FLYING thenflyDownPressed=false end
end)
end
end
end
...
if flyUpPressed~=nil and flyDownPressed~=nil thenbreak end
task.wait(0.5)
end
end)
end
...
if flyUpPressed thenmoveVector=moveVector+Vector3.new(0, flyspeed*50, 0) end
if flyDownPressed thenmoveVector=moveVector-Vector3.new(0, flyspeed*50, 0) end
...
if WPressed thenmoveDirection=moveDirection+camera.CFrame.LookVector*flyspeed end
if SPressed thenmoveDirection=moveDirection-camera.CFrame.LookVector*flyspeed end
if APressed thenmoveDirection=moveDirection-camera.CFrame.RightVector*flyspeed end
if DPressed thenmoveDirection=moveDirection+camera.CFrame.RightVector*flyspeed end
if SpacePressed thenmoveDirection=moveDirection+Vector3.new(0, flyspeed*2, 0) end
if ShiftPressed thenmoveDirection=moveDirection-Vector3.new(0, flyspeed*2, 0) end
BV.Velocity=moveDirection*16
BG.CFrame=camera.CFrame
end
BG:Destroy()
BV:Destroy()
if humanoid thenhumanoid.PlatformStand=false end
end)
end
flyKeyDown=UserInputService.InputBegan:Connect(function(input, processed)
if processed thenreturn end
if input.KeyCode==Enum.KeyCode.W thenWPressed=true
elseifinput.KeyCode==Enum.KeyCode.S thenSPressed=true
elseifinput.KeyCode==Enum.KeyCode.A thenAPressed=true
elseifinput.KeyCode==Enum.KeyCode.D thenDPressed=true
elseifinput.KeyCode==Enum.KeyCode.Space thenSpacePressed=true
elseifinput.KeyCode==Enum.KeyCode.LeftShift thenShiftPressed=true
end
pcall(function() workspace.CurrentCamera.CameraType=Enum.CameraType.Track end)
end)
flyKeyUp=UserInputService.InputEnded:Connect(function(input, processed)
if processed thenreturn end
if input.KeyCode==Enum.KeyCode.W thenWPressed=false
elseifinput.KeyCode==Enum.KeyCode.S thenSPressed=false
elseifinput.KeyCode==Enum.KeyCode.A thenAPressed=false
elseifinput.KeyCode==Enum.KeyCode.D thenDPressed=false
elseifinput.KeyCode==Enum.KeyCode.Space thenSpacePressed=false
elseifinput.KeyCode==Enum.KeyCode.LeftShift thenShiftPressed=false
end
end)
FLY()
end
...
flyUpPressed=false
flyDownPressed=false
if flyKeyDown thenflyKeyDown:Disconnect() flyKeyDown=nil end
if flyKeyUp thenflyKeyUp:Disconnect() flyKeyUp=nil end
if IsOnMobile then
...
if root thenroot.Velocity=Vector3.new(0, 0, 0) end
end
...
task.wait(1)
if IsOnMobile thenmobilefly(Players.LocalPlayer) elsepcfly() end
end
end
...
MainTab:AddToggle("FlyToggle", {Title="Fly",Description="Enable fly mode",Default=false,Callback=function(state)
...
MainTab:AddSlider("FlySpeed", {Title="Fly Speed",Description="Adjust fly speed",Default=70,Min=1,Max=9999,Rounding=0,Callback=function(value) flyspeed=value end})
...
for _, npc in ipairs(ReplicatedStorage.NPCs:GetChildren()) do table.insert(nextBotNames, npc.Name) do end
end
function isNextbotModel(model)
if not model or not model.Name thenreturn false end
for _, name in ipairs(nextBotNames) do if model.Name==name thenreturn true endend
return false
end
function getDistanceFromPlayer(targetPosition)
if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") thenreturn 0 end
...
for distance=maxDistance, 0, do -5 do
...
if sideRay and sideRay.Instance.CanCollide thenisSafe=false break end
end
if isSafe thenreturn groundPos end
end
end
end
return nil
...
for i=1, math.min(3, do #spawnLocations) do
...
if safePosition thenhumanoidRootPart.CFrame=CFrame.new(safePosition) return true end
end
end
end
end
...
if plr~=player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") thentable.insert(validPlayers, plr) end
end
if #validPlayers > 0 then
local character=player.Character
local humanoidRootPart=character and character:FindFirstChild("HumanoidRootPart")
if humanoidRootPart then
for i=1, math.min(3, do #validPlayers) do
...
local character=player.Character
local humanoidRootPart=character and character:FindFirstChild("HumanoidRootPart")
if not humanoidRootPart thenreturn false end
if attachment then
...
local attachment=getAttachment()
if not attachment or not player.Character thenreturn false end
...
if not player.Character thenreturn end
local success=false
if getgenv().featureStates.AntiNextbotSpawnType=="Spawn" then
success=teleportToSpawn()
elseifgetgenv().featureStates.AntiNextbotSpawnType=="Player" then
...
Fluent:Notify({Title="Anti Nextbot Spawn",Content="No safe teleport location found!",Duration=2})
end
end
end
function startAttachmentSearch()
if isSearchingForAttachment thenreturn end
...
if Fluent and Fluent.Notify then
Fluent:Notify({Title="Anti Nextbot Spawn",Content="Attachment found! System now active.",Duration=3})
end
break
end
end
...
if nextbotSpawnConnection thennextbotSpawnConnection:Disconnect() end
nextbotSpawnConnection=RunService.Heartbeat:Connect(function()
if not getgenv().featureStates.AntiNextbotSpawn or not player.Character thenreturn end
if tick()-lastAvoidanceTime < avoidanceCooldown thenreturn end
local attachment=getAttachment()
if not attachment thenreturn end
...
if not getgenv().featureStates.AntiNextbot thenreturn end
local character=Players.LocalPlayer.Character
local humanoidRootPart=character and character:FindFirstChild("HumanoidRootPart")
if not humanoidRootPart thenreturn end
local nextbots={}
...
if hrp thentable.insert(nextbots, model) end
end
end
end
...
elseifgetgenv().featureStates.AntiNextbotTeleportType=="Spawn" then
local spawnsFolder=workspace:FindFirstChild("Game") and workspace.Game:FindFirstChild("Map") and workspace.Game.Map:FindFirstChild("Parts") and workspace.Game.Map.Parts:FindFirstChild("Spawns")
if spawnsFolder then
local spawnLocations=spawnsFolder:GetChildren()
if #spawnLocations > 0 then
...
elseifgetgenv().featureStates.AntiNextbotTeleportType=="Distance" then
...
if antiNextbotConnection thenantiNextbotConnection:Disconnect() end
...
if dist < nearestDistance thennearestDistance=dist nearestNextbot=model end
end
end
end
if npcsFolder then
...
if stopAutoMoneyFarm thenstopAutoMoneyFarm() end
if stopAutoWin thenstopAutoWin() end
farmsSuppressedByAntiNextbot=true
elseifnot isTooClose and farmsSuppressedByAntiNextbot then
getgenv().moneyfarm=previousMoneyFarm
getgenv().ticketfarm=previousTicketFarm
getgenv().autowin=previousAutoWin
if previousMoneyFarm thenif startAutoMoneyFarm thenstartAutoMoneyFarm() endend
...
if antiNextbotConnection thenantiNextbotConnection:Disconnect() antiNextbotConnection=nil end
...
task.wait(1)
if not nextbotSpawnConnection thenstartAntiNextbotSpawn() end
end
end)
local statsFolder=workspace:FindFirstChild("Game") and workspace.Game:FindFirstChild("Stats")
if statsFolder then
...
task.spawn(function()
while true do
if getgenv().featureStates.AntiNextbot thenpcall(handleAntiNextbot) end
task.wait(0.1)
end
end)
task.spawn(function()
...
if Fluent and Fluent.Notify thenFluent:Notify({Title="Anti Nextbot Spawn",Content="Attachment found! System now active.",Duration=3}) end
elseifnot currentAttachment and cachedAttachment then
cachedAttachment=nil
if Fluent and Fluent.Notify thenFluent:Notify({Title="Anti Nextbot Spawn",Content="Attachment lost-System will reactivate when found.",Duration=3}) end
else
cachedAttachment=currentAttachment
end
end
...
if Fluent and Fluent.Notify thenFluent:Notify({Title="Anti Nextbot System",Content="System loaded successfully!",Duration=2}) end
restartSystemOnEvents()
CombatTab:AddToggle("AntiNextbot", {Title="Anti-Nextbot",Description="Automatically teleport away from nearby Nextbots (farms pause if too close)",Default=getgenv().featureStates.AntiNextbot,Callback=function(state) getgenv().featureStates.AntiNextbot=state if state thenstartAntiNextbot() elsestopAntiNextbot() endend})
CombatTab:AddDropdown("AntiNextbotTeleportType", {Title="Anti-Nextbot Teleport Type",Description="Choose how to teleport when avoiding Nextbots",Values={"Players", "Spawn", "Distance"},Default=getgenv().featureStates.AntiNextbotTeleportType,Callback=function(value) getgenv().featureStates.AntiNextbotTeleportType=value end})
CombatTab:AddInput("AntiNextbotDistance", {Title="Anti-Nextbot Distance",Description="Distance threshold for Nextbot detection",Default=tostring(getgenv().featureStates.AntiNextbotDistance),Placeholder=tostring(getgenv().featureStates.AntiNextbotDistance),Numeric=true,Callback=function(value) local num=tonumber(value) if num and num > 0 thengetgenv().featureStates.AntiNextbotDistance=num endend})
CombatTab:AddInput("DistanceTeleport", {Title="Distance Teleport",Description="How far to teleport when using Distance mode",Default=tostring(getgenv().featureStates.DistanceTeleport),Placeholder=tostring(getgenv().featureStates.DistanceTeleport),Numeric=true,Callback=function(value) local num=tonumber(value) if num and num > 0 thengetgenv().featureStates.DistanceTeleport=num endend})
CombatTab:AddToggle("AntiNextbotSpawn", {Title="Anti Nextbot Spawn",Description="Automatically avoid Nextbot spawn areas",Default=false,Callback=function(state) getgenv().featureStates.AntiNextbotSpawn=state if state thenstartAntiNextbotSpawn() startAttachmentSearch() if not getAttachment() thenif Fluent and Fluent.Notify thenFluent:Notify({Title="Anti Nextbot Spawn",Content="System enabled-Searching for attachment...",Duration=3}) endelseifFluent and Fluent.Notify thenFluent:Notify({Title="Anti Nextbot Spawn",Content="System enabled-Attachment found!",Duration=3}) endendelseifnextbotSpawnConnection thennextbotSpawnConnection:Disconnect() nextbotSpawnConnection=nil endisSearchingForAttachment=false endend})
CombatTab:AddDropdown("AvoidanceMode", {Title="Avoidance Mode",Description="Choose how to avoid Nextbot spawn",Values={"Spawn", "Player", "Distance"},Default="Spawn",Callback=function(value) getgenv().featureStates.AntiNextbotSpawnType=value end})
CombatTab:AddInput("AvoidanceDistance", {Title="Avoidance Distance",Description="Distance to trigger avoidance (studs)",Default=tostring(getgenv().featureStates.AntiNextbotSpawnDistance),Placeholder="40",Numeric=true,Callback=function(value) local distance=tonumber(value) if distance and distance > 0 thengetgenv().featureStates.AntiNextbotSpawnDistance=distance endend})
CombatTab:AddInput("TeleportDistance", {Title="Teleport Distance",Description="How far to teleport in Distance mode (studs)",Default=tostring(getgenv().featureStates.AntiNextbotTeleportDistance),Placeholder="20",Numeric=true,Callback=function(value) local distance=tonumber(value) if distance and distance > 0 thengetgenv().featureStates.AntiNextbotTeleportDistance=distance endend})
MiscTab:AddButton({Title="Redeem All Code",Description="Redeem All code That are listed in Wikipedia",Callback=function()
if getgenv().EvadeCode thengame:GetService("Players").LocalPlayer.PlayerGui.Global.Messages.Use:Fire("You already used this", "Error") return end
...
if success thenreturn content elsereturn nil end
end
function extractTRData(htmlContent)
if not htmlContent thenreturn {} end
local tableRows={}
...
if statusValue=="true" or statusValue=="false" thenrowData.spanStatus=statusValue end
end
if not rowData.spanStatus then
local truePos=trContent:lower():find("true")
local falsePos=trContent:lower():find("false")
if truePos thenrowData.spanStatus="true" elseiffalsePos thenrowData.spanStatus="false" end
end
if rowData.code and #rowData.code > 0 thentable.insert(tableRows, rowData) end
end
return tableRows
end
function useCodeWithFeedback(code)
local success, response=pcall(function() return RemoteEvent:InvokeServer(code) end)
if not success thenMessageEvent:Fire("Failed to fetch our Wikipedia content, please try again later", "Error") return end
if response==nil thenMessageEvent:Fire("Redeemed!", "Success")
elseiftype(response)=="boolean" then
if response thenMessageEvent:Fire("Accepted!", "Success") elseMessageEvent:Fire("Invalid", "Error") end
elseiftype(response)=="string" then
local lower=response:lower()
if lower:find("success") or lower:find("accepted") or lower:find("redeemed") or lower:find("reward") thenMessageEvent:Fire(response, "Success")
elseiflower:find("invalid") or lower:find("error") or lower:find("failed") or lower:find("already") thenMessageEvent:Fire(response, "Error")
elseMessageEvent:Fire(response, "Success") end
elseiftype(response)=="table" then
if response.success==true thenMessageEvent:Fire(response.message or "Success", "Success")
elseifresponse.success==false thenMessageEvent:Fire(response.message or "Failed", "Error")
elseifresponse.error thenMessageEvent:Fire(response.error, "Error")
elseifresponse.message thenMessageEvent:Fire(response.message, "Info")
elseMessageEvent:Fire("Processed", "Info") end
elseMessageEvent:Fire("Response: " .. tostring(response), "Info") end
end
function main()
MessageEvent:Fire("Starting code extraction...", "Info")
local htmlContent=getContent(URL)
if not htmlContent thenMessageEvent:Fire("Failed to fetch our Wikipedia content", "Error") return end
local tableRows=extractTRData(htmlContent)
if #tableRows==0 thenMessageEvent:Fire("No table rows found", "Error") return end
...
if status=="true" thentable.insert(trueCodes, row.code)
elseifstatus=="false" thentable.insert(falseCodes, row.code)
elsetable.insert(unknownCodes, row.code) end
end
...
task.wait(5)
local validCodes={}
for _, code in ipairs(trueCodes) do table.insert(validCodes, code) do end
for _, code in ipairs(unknownCodes) do table.insert(validCodes, code) do end
if #validCodes==0 thenMessageEvent:Fire("No valid codes to try", "Error") return end
...
if not player thenreturn false end
if not playersPath thenreturn false end
for _, model in pairs(playersPath:GetChildren()) do if model.Name==player.Name thenreturn true endend
return false
end
local function shouldApplySettings()
if not gameStatsPath thenreturn false end
...
if rawget(obj, field)==nil thenok=false break end
end
if ok thentable.insert(matched, obj) end
end
end
return matched
end
local function applyToTables(callback)
if not isPlayerModelPresent() thenreturn end
local targets=getMatchingTables()
if #targets==0 thenreturn end
if getgenv().ApplyMode=="Optimized" then
task.spawn(function()
for i, tableObj in ipairs(targets) do
if tableObj and typeof(tableObj)=="table" thenpcall(callback, tableObj) end
if i%3==0 thentask.wait() end
end
end)
else
for i, tableObj in ipairs(targets) do
...
MiscTab:AddInput("Speed", {Title="Speed",Description="Set movement speed",Default="1500",Placeholder="1500",Numeric=true,Callback=function(input)
local val=tonumber(input)
if not val thenreturn end
if val < 1450 thenreturn end
currentSettings.Speed=input
applyToTables(function(obj) obj.Speed=val end)
end})
MiscTab:AddInput("JumpCap", {Title="Jump Cap",Description="Set jump cap value",Default="1",Placeholder="1",Numeric=true,Callback=function(input)
local val=tonumber(input)
if not val thenreturn end
if val < 0.1 thenreturn end
currentSettings.JumpCap=input
applyToTables(function(obj) obj.JumpCap=val end)
end})
MiscTab:AddInput("StrafeSpeed", {Title="Strafe speed",Description="Set air strafe acceleration",Default="187",Placeholder="187",Numeric=true,Callback=function(input)
local val=tonumber(input)
if not val thenreturn end
if val < 1 thenreturn end
currentSettings.AirStrafeAcceleration=input
applyToTables(function(obj) obj.AirStrafeAcceleration=val end)
end})
MiscTab:AddDropdown("ApplyMethod", {Title="Select Apply Method",Values={"Unoptimized", "Optimized"},Default="Unoptimized",Callback=function(value) getgenv().ApplyMode=value end})
...
if not itemsFolder thenreturn end
local emotesFolder=itemsFolder:FindFirstChild("Emotes")
if not emotesFolder thenreturn end
for _, module in ipairs(emotesFolder:GetChildren()) do
if module:IsA("ModuleScript") then
local ok, data=pcall(require, module)
...
MiscTab:AddInput("EmoteSpeedValue", {Title="Emote Speed Value",Description="Set emote speed multiplier",Default=tostring(getgenv().featureStates.EmoteSpeedValue),Placeholder="1500",Numeric=true,Callback=function(value)
local num=tonumber(value)
if not num or num<=0 thenreturn end
getgenv().featureStates.EmoteSpeedValue=num
local applied=num/1000
applyEmoteSpeed(applied)
Fluent:Notify({Title="Emote Speed",Content="Emote speed set to " .. num,Duration=2})
end})
MiscTab:AddButton({Title="Reset Emote Speed",Description="Reset emote speed to original values",Callback=function()
...
task.spawn(function()
while true do
if fflagEnabled thenSetFFlag(1000) end
task.wait(1)
end
end)
...
MiscTab:AddDropdown("LagSwitchMode",{Title="Lag Switch Mode",Values={"Normal","Demon"},Default=getgenv().LagSwitchMode,Callback=function(value) getgenv().LagSwitchMode=value if value=="Demon" thenfflagEnabled=true elsefflagEnabled=false endend})
MiscTab:AddInput("LagSwitchDuration",{Title="Lag Duration",Default=tostring(getgenv().LagSwitchDuration),Placeholder="0.5",Numeric=true,Callback=function(value) local num=tonumber(value) if num and num > 0 thengetgenv().LagSwitchDuration=num elsegetgenv().LagSwitchDuration=0.5 endend})
...
elseifcurrentState=="EmotingSlide" then
...
if SlideEnabled thenreturn end
SlideEnabled=true
RefreshCachedTables()
UpdatePlayerModel()
...
if not SlideEnabled thenreturn end
SlideEnabled=false
SetFriction(5)
if HeartbeatConnection then
HeartbeatConnection:Disconnect()
...
MiscTab:AddToggle("InfiniteSlide", {Title="Infinite Slide",Description="Enable infinite slide mode",Default=false,Callback=function(state) if state thenStartInfiniteSlide() elseStopInfiniteSlide() endend})
MiscTab:AddInput("SlideFriction", {Title="Slide Friction",Description="Set slide friction value (negative numbers only)",Default="-8",Placeholder="-8",Numeric=true,Callback=function(value)
local num=tonumber(value)
if num and num < 0 then
SlideFrictionValue=num
if SlideEnabled and PlrModel then
local currentState=PlrModel:GetAttribute("State")
if currentState=="EmotingSlide" thenSetFriction(SlideFrictionValue) end
end
end
end})
...
if task.ResourceInfo and task.ResourceInfo.Type=="Clip" thenportalTask=task break end
end
if not portalTask thenportalTask=Breacher.Tasks[2] end
...
local newDisabled={}
for _, action in ipairs(blueShoot.CooldownInfo.DisabledActions) do if action~="ADS" thentable.insert(newDisabled, action) do endend
...
local newDisabled={}
for _, action in ipairs(yellowShoot.CooldownInfo.DisabledActions) do if action~="ADS" thentable.insert(newDisabled, action) do endend
...
if Breacher.Actions.ADS.Zoom thenBreacher.Actions.ADS.Zoom=nil end
return true
end)
if success then
Fluent:Notify({Title="Breacher (Portal Gun)",Content="Portal Gun Successfully upgraded! \n✓ Infinite charges \n✓ Maximum range \n✓ Instant reload",Duration=6})
else
Fluent:Notify({Title="Breacher Error",Content="Error: " .. tostring(result),Duration=5})
end
end})
MiscTab:AddButton({Title="Grapplehook",Description="Upgrade Grapplehook tool",Callback=function()
local success, result=pcall(function()
...
if success then
Fluent:Notify({Title="Grapplehook",Content="Grapplehook Successfully upgraded! \n✓ Infinite charges \n✓ Super speed \n✓ No gravity",Duration=6})
else
Fluent:Notify({Title="Grapplehook Error",Content="Error: " .. tostring(result),Duration=5})
end
end})
MiscTab:AddButton({Title="Smoke Grenade",Description="Upgrade Smoke Grenade",Callback=function()
local success, result=pcall(function()
...
if success then
Fluent:Notify({Title="Smoke Grenade",Content="Smoke Grenade Improved! \n✓ Infinite Grenades \n✓ Instant Reload \n✓ Massive Smoke Cloud",Duration=6})
else
Fluent:Notify({Title="Smoke Grenade Error",Content="Error: " .. tostring(result),Duration=5})
end
end})
MiscTab:AddSection("Gravity", "apple")
getgenv().GravityEnabled=false
getgenv().GravityValue=50
MiscTab:AddInput("GravityValue", {Title="Gravity Value",Description="Set custom gravity value",Default=tostring(getgenv().GravityValue),Placeholder=tostring(getgenv().GravityValue),Numeric=true,Callback=function(val)
local n=tonumber(val)
if n and n > 0 then
getgenv().GravityValue=n
if getgenv().GravityEnabled thenworkspace.Gravity=n end
end
end})
...
if gameProcessedEvent thenreturn end
...
if not touchGui thenreturn end
local touchControlFrame=touchGui:WaitForChild("TouchControlFrame")
if not touchControlFrame thenreturn end
local jumpButton=touchControlFrame:WaitForChild("JumpButton")
if not jumpButton thenreturn end
jumpButton.MouseButton1Down:Connect(function()
...
if not autoOn thenreturn end
local character=player.Character
if not character or not character:FindFirstChild("Humanoid") thenreturn end
local humanoid=character.Humanoid
if mode=="Spam" then
fireKeybind(spamDown, "Crouch")
spamDown=not spamDown
elseifmode=="Normal" then
...
if ok thenreturn spd end
return nil
end
local function IsOnGroundBack()
local char=LocalPlayer.Character
if not char thenreturn false end
local humanoid=char:FindFirstChild("Humanoid")
local root=char:FindFirstChild("HumanoidRootPart")
if not humanoid or not root thenreturn false end
local state=humanoid:GetState()
if state==Enum.HumanoidStateType.Jumping or state==Enum.HumanoidStateType.Freefall or state==Enum.HumanoidStateType.Swimming thenreturn false end
local raycastParams=RaycastParams.new()
raycastParams.FilterType=Enum.RaycastFilterType.Blacklist
...
if not raycastResult thenreturn false end
...
if not checkcaller() and countingEnabled and speedometer and self==speedometer and idx=="Text" thenreturn end
...
if speedometer thenspeedometer.Text=tostring(truncate1Decimal(currentSpeed)) end
airTotalTime=0
end
wasAir=isAir
if getgenv().BackJumpEnabled then
...
if activeBV thenactiveBV:Destroy() end
local lookDir=camera.CFrame.LookVector
lookDir=Vector3.new(-lookDir.X, 0, -lookDir.Z)
if lookDir.Magnitude~=0 thenlookDir=lookDir.Unit end
...
MiscTab:AddInput("BackJumpSpeed", {Title="Back Jump Speed",Description="Set back jump speed (1-500)",Default="50",Placeholder="50",Numeric=true,Callback=function(input)
local val=tonumber(input)
...
if not checkcaller() and countingEnabledFront and speedometerFront and self==speedometerFront and idx=="Text" thenreturn end
...
if speedometerFront thenspeedometerFront.Text=tostring(truncate1DecimalFront(currentSpeedFront)) end
airTotalTimeFront=0
end
wasAirFront=isAir
if getgenv().FrontJumpEnabled then
...
if activeBVFront thenactiveBVFront:Destroy() end
local lookDir=camera.CFrame.LookVector
lookDir=Vector3.new(lookDir.X, 0, lookDir.Z)
if lookDir.Magnitude~=0 thenlookDir=lookDir.Unit end
local bv=Instance.new("BodyVelocity")
...
MiscTab:AddInput("FrontJumpSpeed", {Title="Front Jump Speed",Description="Set front jump speed (1-500)",Default="50",Placeholder="50",Numeric=true,Callback=function(input)
local val=tonumber(input)
if val and val > 0 and val<=500 then
...
if not char thenreturn end
local hrp=char:FindFirstChild("HumanoidRootPart")
local isDowned=char:GetAttribute("Downed")
if not isDowned thenreturn end
...
elseiffeatureStates.SelfReviveMethod=="Revive" then
if hrp thenlastSavedPosition=hrp.Position end
task.spawn(function()
task.wait(3)
...
task.wait(0.5)
if (newHRP.Position-lastSavedPosition).Magnitude > 1 thenlastSavedPosition=nil end
end
end)
end
end
local function setupAutoRevive(char)
if AutoSelfReviveConnection thenAutoSelfReviveConnection:Disconnect() end
AutoSelfReviveConnection=char:GetAttributeChangedSignal("Downed"):Connect(function()
if char:GetAttribute("Downed") thendoRevive(char) end
end)
end
if respawnConnection thenrespawnConnection:Disconnect() end
respawnConnection=LocalPlayer.CharacterAdded:Connect(function(newChar)
task.wait(1)
if featureStates.AutoSelfRevive thensetupAutoRevive(newChar) end
end)
...
if state and LocalPlayer.Character thensetupAutoRevive(LocalPlayer.Character) end
end})
if LocalPlayer.Character and featureStates.AutoSelfRevive thensetupAutoRevive(LocalPlayer.Character) end
...
MiscTab:AddInput("CarryDelay", {Title="Carry Delay (s)",Description="Set auto carry delay",Default=tostring(getgenv().AutoCarryDelay),Placeholder="1",Numeric=true,Callback=function(val) local n=tonumber(val) if n and n > 0 thengetgenv().AutoCarryDelay=n endend})
MiscTab:AddInput("ReviveDelay", {Title="Revive Delay (s)",Description="Set auto revive delay",Default=tostring(getgenv().AutoReviveDelay),Placeholder="1",Numeric=true,Callback=function(val) local n=tonumber(val) if n and n > 0 thengetgenv().AutoReviveDelay=n endend})
...
if HUM thenORIGINAL_DESC=HUM:GetAppliedDescription() end
end
local function applyAvatarToCharacter(char, name, id, desc)
task.spawn(function()
local HUM=char:WaitForChild("Humanoid", 10)
if not HUM thenreturn end
for _, v in char:GetDescendants() do
if v:IsA("Accessory") or v:IsA("Hat") thenv:Destroy() end
end
for _, v in char:GetChildren() do
if v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") thenv:Destroy() end
end
local BC=HUM:FindFirstChildOfClass("BodyColors")
if BC thenBC:Destroy() end
for _, partName in {"Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"} do
local part=char:FindFirstChild(partName)
if part then
for _, v in part:GetChildren() do
if v:IsA("SpecialMesh") thenv:Destroy() end
end
end
end
...
if not success or not name thenreturn end
userName=name
else
local success, id=pcall(function() return Players:GetUserIdFromNameAsync(input) end)
if not success or not id thenreturn end
userId=id
local success2, name=pcall(function() return Players:GetNameFromUserIdAsync(userId) end)
if not success2 or not name thenreturn end
userName=name
end
local success, desc=pcall(function() return Players:GetHumanoidDescriptionFromUserId(userId) end)
if not success or not desc thenreturn end
CURRENT_AVATAR={id=userId,name=userName,desc=desc}
if LP.Character thenapplyAvatarToCharacter(LP.Character, userName, userId, desc) end
LP.CharacterAdded:Connect(function(char)
if CURRENT_AVATAR and CURRENT_AVATAR.id==userId thenapplyAvatarToCharacter(char, userName, userId, desc) end
end)
end
...
VisualTab:AddInput("AvatarInput", {Title="Username/User ID",Description="Enter Roblox username or user ID",Default="",Placeholder="Roblox or 123456789",Callback=function(value) avatarInputValue=value if value and value~="" thenchangeAvatar(value) endend})
VisualTab:AddButton({Title="Apply Avatar",Description="Apply avatar from entered username/ID",Callback=function() if avatarInputValue and avatarInputValue~="" thenchangeAvatar(avatarInputValue) endend})
...
if not head thenreturn end
head.Transparency=1
head.CanCollide=false
local face=head:FindFirstChild("face")
if face thenface:Destroy() end
...
if not head thenreturn end
if headlessMesh thenheadlessMesh:Destroy() headlessMesh=nil end
head.Transparency=0
head.CanCollide=true
end
local function applyKorblox(character)
local rightLeg=character:FindFirstChild("Right Leg") or character:FindFirstChild("RightUpperLeg")
if not rightLeg thenreturn end
if not originalLegColor thenoriginalLegColor=rightLeg.Color end
for _, child in ipairs(rightLeg:GetChildren()) do
if child:IsA("SpecialMesh") or child:IsA("CharacterMesh") thenchild:Destroy() end
end
...
if originalLegColor thenrightLeg.Color=originalLegColor originalLegColor=nil end
end
LP.CharacterAdded:Connect(function(char)
task.wait(0.5)
local head=char:FindFirstChild("Head")
if headlessEnabled thenapplyHeadless(head) end
if korbloxEnabled thenapplyKorblox(char) end
end)
VisualTab:AddToggle("Headless", {Title="Headless",Description="Enable headless appearance",Default=false,Callback=function(state)
headlessEnabled=state
local char=LP.Character
if not char thenreturn end
local head=char:FindFirstChild("Head")
if state thenapplyHeadless(head) elseremoveHeadless(head) end
end})
VisualTab:AddToggle("Korblox", {Title="Korblox",Description="Enable Korblox leg",Default=false,Callback=function(state)
korbloxEnabled=state
local char=LP.Character
if not char thenreturn end
if state thenapplyKorblox(char) elseremoveKorblox(char) end
end})
VisualTab:AddSection("FOV CHANGER", "apple")
VisualTab:AddSlider("FOV", {Title="Field of View",Description="Adjust your field of view",Default=70,Min=1,Max=270,Rounding=0,Callback=function(value)
local num=tonumber(value)
if num thenpcall(function() game:GetService("ReplicatedStorage").Events.Data.ChangeSetting:InvokeServer(2, num) end) end
end})
...
if cameraStretchConnection thencameraStretchConnection:Disconnect() cameraStretchConnection=nil end
...
VisualTab:AddToggle("CameraStretch", {Title="Camera Stretch",Description="Enable camera stretch effect",Default=false,Callback=function(state)
if state thenupdateCameraStretch() elseifcameraStretchConnection thencameraStretchConnection:Disconnect() cameraStretchConnection=nil endend
end})
VisualTab:AddInput("StretchHorizontal", {Title="Camera Stretch Horizontal",Description="Set horizontal stretch value",Default=tostring(stretchHorizontal),Placeholder="0.80",Numeric=true,Callback=function(value)
local num=tonumber(value)
if num then
stretchHorizontal=num
if cameraStretchConnection thenupdateCameraStretch() end
end
end})
VisualTab:AddInput("StretchVertical", {Title="Camera Stretch Vertical",Description="Set vertical stretch value",Default=tostring(stretchVertical),Placeholder="0.80",Numeric=true,Callback=function(value)
local num=tonumber(value)
if num then
stretchVertical=num
if cameraStretchConnection thenupdateCameraStretch() end
...
VisualTab:AddInput("FakeStreaks", {Title="Fake Streaks",Description="Set fake streak value",Default="",Placeholder="Enter streak value",Numeric=true,Callback=function(value)
local num=tonumber(value)
if num then
pcall(function()
LocalPlayer:SetAttribute("Streak", num)
if Fluent and Fluent.Notify thenFluent:Notify({Title="Streak",Content=string.format("Streak set to %d", num),Duration=3}) end
end)
end
end})
...
VisualTab:AddInput("CurrentCosmetics", {Title="Current Cosmetics",Description="Enter current cosmetic name",Default="",Placeholder="Enter current cosmetic name",Callback=function(v) cosmetic1=v if not isSwapped thenoriginalCosmetic1=v endend})
VisualTab:AddInput("SelectCosmetics", {Title="Select Cosmetics",Description="Enter cosmetic to swap with",Default="ToxicInferno",Placeholder="Enter cosmetic to swap with",Callback=function(v) cosmetic2=v if not isSwapped thenoriginalCosmetic2=v endend})
VisualTab:AddButton({Title="Apply Cosmetics",Description="Swap cosmetics",Callback=function()
pcall(function()
if cosmetic1=="" or cosmetic2=="" or cosmetic1==cosmetic2 thenreturn end
...
if score > bestScore thenbestScore=score bestMatch=c.Name end
end
return bestMatch
end
...
if not a or not b thenreturn end
...
VisualTab:AddButton({Title="Reset Cosmetics",Description="Reset swapped cosmetics",Callback=function()
pcall(function()
if not isSwapped thenreturn end
if originalCosmetic1=="" or originalCosmetic2=="" thenreturn end
local Cosmetics=ReplicatedStorage:WaitForChild("Items"):WaitForChild("Cosmetics")
function normalize(str) return str:gsub("%s+", ""):lower() end
function findSimilar(name)
for _, c in ipairs(Cosmetics:GetChildren()) do
if normalize(c.Name)==normalize(name) thenreturn c.Name end
end
return name
end
...
VisualTab:AddSection("emote changer", "apple")
...
if partialName=="" thenreturn nil end
local normalizedSearch=string.lower(partialName:gsub("%s+", ""))
for _, emote in ipairs(allEmotesList) do
if emote.normalized:find(normalizedSearch, 1, true) thenreturn emote.displayName end
end
return nil
end
function fireSelect(emoteName)
if not currentTag thenreturn end
local tagNumber=tonumber(currentTag)
if not tagNumber or tagNumber < 0 or tagNumber > 255 thenreturn end
if not emoteName or emoteName=="" thenreturn end
local bufferData=buffer.create(2)
buffer.writeu8(bufferData, 0, tagNumber)
buffer.writeu8(bufferData, 1, 17)
if remoteSignal thenfiresignal(remoteSignal, bufferData, {emoteName}) end
end
...
if r15Visual thenhumanoid=r15Visual:WaitForChild("Visual_Humanoid", 5) end
else
...
if player.Character thensetupHumanoidListeners(player.Character) end
...
if emoteModelScript thenreturn emoteModelScript end
for _, script in pairs(player.PlayerScripts:GetDescendants()) do
if script.Name=="EmoteModel" thenemoteModelScript=script return script end
end
...
if not text thenreturn "" end
if not normalizedCache[text] thennormalizedCache[text]=string.lower(text:gsub("%s+", "")) end
return normalizedCache[text]
end
function findEmoteModuleByDisplayName(displayName)
if displayName=="NONE" or not displayName thenreturn nil end
if emoteNameCache[displayName]~=nil thenreturn emoteNameCache[displayName] end
local emotesFolder=ReplicatedStorage:FindFirstChild("Items")
if not emotesFolder thenemoteNameCache[displayName]=nil return nil end
emotesFolder=emotesFolder:FindFirstChild("Emotes")
if not emotesFolder thenemoteNameCache[displayName]=nil return nil end
...
if emoteFrame thenemoteFrame=nil end
end
function restoreOriginalEmotes()
if not emoteModelScript thenfindEmoteModelScript() end
if not emoteModelScript or not emoteFrame thenreturn end
...
if not emoteWheel thenreturn end
function processSlot(emoteSlot, key)
if not emoteSlot thenreturn end
...
if original then
if viewportFrame:FindFirstChild("WorldModel") thenviewportFrame.WorldModel:Destroy() end
...
if emoteModule thenemoteModelFunction(viewportFrame, original.emoteName) end
...
if emoteWheel2 thenfor i=1, 6 do processSlot(emoteWheel2:FindFirstChild("Emote"..i), "Wheel2_Emote"..i) endend
end
function replaceEmotesFrame()
if not replacementEnabled or not emoteDataSaved or not emoteFrame thenreturn false end
if not emoteModelScript thenfindEmoteModelScript() end
if not emoteModelScript thenreturn false end
local emoteModelFunction=require(emoteModelScript)
local emoteWheel=emoteFrame:FindFirstChild("Wheel")
local emoteWheel2=emoteFrame:FindFirstChild("Wheel2")
if not emoteWheel thenreturn false end
local anyPairsConfigured=false
function processEmoteSlot(emoteSlot, wheelName, i)
if not emoteSlot thenreturn end
local textLabel=emoteSlot:FindFirstChild("TextLabel")
if not textLabel thenreturn end
...
if emoteWheel2 thenfor i=1, 6 do processEmoteSlot(emoteWheel2:FindFirstChild("Emote"..i), "Wheel2", i) endend
return anyPairsConfigured
end
function saveOriginalEmoteData(frame)
if not frame thenreturn end
originalEmoteData={}
...
if emoteWheel2 thenfor i=1, 6 do saveSlot(emoteWheel2:FindFirstChild("Emote"..i), "Wheel2_Emote"..i) endend
emoteDataSaved=true
end
function readTagFromFolder(folder)
if not folder thenreturn nil end
local attributeValue=folder:GetAttribute("Tag")
if attributeValue~=nil thenreturn attributeValue end
local tagValue=folder:FindFirstChild("Tag")
if tagValue and tagValue:IsA("ValueBase") thenreturn tagValue.Value end
return nil
end
...
currentTag=nil
emoteFrame=nil
if reapplyThread thentask.cancel(reapplyThread) reapplyThread=nil end
end
function handleSingleRespawn()
local now=tick()
if respawnInProgress and (now-lastRespawnTime) < 2 thenreturn end
...
if currentTag then
local tagNumber=tonumber(currentTag)
if tagNumber and tagNumber>=0 and tagNumber<=255 thenbreak end
end
end
end
task.wait(0.1)
...
if not replacementEnabled or not emoteDataSaved thenreturn end
...
if cleaned~="" thensearchEmote(cleaned) end
end})
end
...
Description="Higher Value may Broke emote animation recommendUse 1-3 (0 or 'Random' for random)",
Default="0",
Placeholder="0",
Numeric=true,
Callback=function(v)
...
if emoteFrame and emoteDataSaved thenrestoreOriginalEmotes() end
if replacementEnabled and emoteFrame and emoteDataSaved thenreplaceEmotesFrame() end
end})
...
for i=1, 6 do
if currentEmotes[i]~="" or selectEmotes[i]~="" thenhasAnyEmote=true break end
end
if not hasAnyEmote thenreturn end
function normalizeEmoteName(name) return name:gsub("%s+", ""):lower() end
function isValidEmote(emoteName)
if emoteName=="" thenreturn false, "" end
...
for _, emoteModule in ipairs(emotesFolder:GetChildren()) do
if emoteModule:IsA("ModuleScript") then
if normalizeEmoteName(emoteModule.Name)==normalizedInput thenreturn true, emoteModule.Name end
end
end
end
end
...
emoteFrame=getEmoteFrame()
if not emoteFrame thenreturn end
saveOriginalEmoteData(emoteFrame)
restoreOriginalEmotes()
if replacementEnabled thenreplaceEmotesFrame() end
end})
VisualTab:AddButton({Title="Reset All Emotes",Description="Reset all emote mappings",Callback=function()
if emoteFrame thenrestoreOriginalEmotes() end
for i=1, 6 do currentEmotes[i]="" selectEmotes[i]="" end
emoteNameCache={}
normalizedCache={}
cleanUpLastEmoteFrame()
end})
if player.Character thentask.spawn(handleSingleRespawn) end
...
if child.Name==player.Name thentask.wait(0.5) handleSingleRespawn() end
end)
workspace.Game.Players.ChildRemoved:Connect(function(child)
if child.Name==player.Name thencurrentTag=nil cleanUpLastEmoteFrame() end
end)
end
...
AutoFarmsTab:AddToggle("AntiAFK", {Title="Anti AFK",Description="Prevent being kicked for AFK",Default=false,Callback=function(state) getgenv().AntiAFKEnabled=state if state thenstartAntiAFK() endend})
...
Fluent:Notify({Title="Auto Farm Lupen",Content="Following Lupen...",Duration=3})
end
local function stopAutoLupen()
if AutoLupenConnection thenAutoLupenConnection:Disconnect() AutoLupenConnection=nil end
Fluent:Notify({Title="Auto Farm Lupen",Content="Stopped.",Duration=3})
end
AutoFarmsTab:AddToggle("AutoLupenToggle", {Title="Auto Farm Lupen",Default=false,Callback=function(state) AutoLupenEnabled=state if state thenstartAutoLupen() elsestopAutoLupen() endend})
...
AutoFarmsTab:AddToggle("AutoTicketFarm", {Title="Auto Ticket Farm",Description="Automatically collect tickets",Default=false,Callback=function(state) getgenv().AutoTicketFarm=state end})
...
if ticket thenhrp.CFrame=CFrame.new(ticket.Position) end
end
end
end
end)
getgenv().AutoMoneyFarm=false
AutoFarmsTab:AddToggle("AutoMoneyFarm", {Title="Auto Money Farm",Description="Automatically farm money",Default=false,Callback=function(state) getgenv().AutoMoneyFarm=state end})
RunService.Heartbeat:Connect(function()
...
if not humanoid or not rootPart thenreturn end
bouncing=true
climbTime=tick()
if connection thenconnection:Disconnect() end
connection=RunService.Heartbeat:Connect(function()
if not bouncing or not character or not character.Parent then
if connection thenconnection:Disconnect() connection=nil end
return
end
...
local humanoid=character and character:FindFirstChildOfClass("Humanoid")
if not humanoid thenreturn end
humanoid.StateChanged:Connect(function(oldState, newState)
if newState==Enum.HumanoidStateType.Climbing then
if BounceSystem.Config.Enabled thenstartBouncing(character) end
elseifoldState==Enum.HumanoidStateType.Climbing and newState~=Enum.HumanoidStateType.Climbing then
stopBouncing()
end
end)
...
MiscTab:AddToggle("BounceMultiplier", {Title="Bounce Multiplier",Description="Enable bounce multiplier on climbing",Default=false,Callback=function(state)
BounceSystem.Config.Enabled=state
if state then
if LocalPlayer.Character thensetupBounce(LocalPlayer.Character) end
else
stopBouncing()
end
end})
MiscTab:AddInput("BounceValue", {Title="Multiplier value",Description="Set bounce multiplier value",Default=tostring(BounceSystem.Config.VelocityMultiplier),Placeholder="80",Numeric=true,Callback=function(value)
local num=tonumber(value)
if num then
...
v.Material=Enum.Material.SmoothPlastic
v.Reflectance=0
elseifv:IsA("Decal") or v:IsA("Texture") then
v:Destroy()
elseifv:IsA("ParticleEmitter") then
v.Enabled=false
end
end
